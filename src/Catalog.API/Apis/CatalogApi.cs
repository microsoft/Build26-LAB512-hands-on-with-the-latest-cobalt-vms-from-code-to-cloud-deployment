...
public static class CatalogApi
{
        ...
        [Description("The text string to use when search for related items in the catalog"), Required, MinLength(1)] string text)
    {
        ...
        TimingLog.Write($"[Catalog] [SemanticSearch] START: \"{text}\"");
        var pageSize = paginationRequest.PageSize;
        var pageIndex = paginationRequest.PageIndex;

        if (!services.CatalogAI.IsEnabled)
        {
            services.Logger.LogInformation("[SemanticSearch] AI not enabled, falling back to name search for: '{Text}'", text);
        ...
        var embMs = embSw.ElapsedMilliseconds;

        if (vector is null)
        {
            services.Logger.LogWarning("[SemanticSearch] Embedding returned null for: '{Text}'", text);
            return await GetItemsByName(paginationRequest, services, text);
        }

        services.Logger.LogInformation("[SemanticSearch] Embedding generated in {EmbMs}ms for: '{Text}'", embMs, text);
        TimingLog.Write($"[Catalog] [SemanticSearch] Embedding in {embMs}ms");
        ...
        services.Logger.LogInformation("[SemanticSearch] DB query in {DbMs}ms, {Count} results, total={TotalMs}ms for: '{Text}'",
            dbMs, itemsOnPage.Count, totalSw.ElapsedMilliseconds, text);
        TimingLog.Write($"[Catalog] [SemanticSearch] END in {totalSw.ElapsedMilliseconds}ms (emb={embMs}ms, db={dbMs}ms, {itemsOnPage.Count} results)");
