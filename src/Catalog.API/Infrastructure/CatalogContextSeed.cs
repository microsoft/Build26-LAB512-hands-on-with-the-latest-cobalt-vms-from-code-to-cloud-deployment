    ...
    ILogger<CatalogContextSeed> logger) : IDbSeeder<CatalogContext>
{
    public async Task SeedAsync(CatalogContext context)
    {
        ...
        else if (catalogAI.IsEnabled)
        {
            // Backfill embeddings for items that were seeded without an AI provider
            var itemsMissingEmbeddings = await context.CatalogItems
                .Where(c => c.Embedding == null)
                .ToListAsync();

            if (itemsMissingEmbeddings.Count > 0)
            {
                logger.LogInformation("Backfilling {NumItems} embeddings for existing catalog items", itemsMissingEmbeddings.Count);
                IReadOnlyList<Vector>? embeddings = await catalogAI.GetEmbeddingsAsync(itemsMissingEmbeddings);
                if (embeddings is not null)
                {
                    for (int i = 0; i < itemsMissingEmbeddings.Count; i++)
                    {
                        itemsMissingEmbeddings[i].Embedding = embeddings[i];
                    }
                    await context.SaveChangesAsync();
                    logger.LogInformation("Backfilled {NumItems} embeddings successfully", itemsMissingEmbeddings.Count);
                }
            }
        }
    }
