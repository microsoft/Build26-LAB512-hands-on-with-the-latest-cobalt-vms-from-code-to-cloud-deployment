...
public static class Extensions
{
    ...
    public static void AddAuthenticationServices(this IHostApplicationBuilder builder)
    {
        ...
        var identityUrlExternal = configuration["IdentityUrlExternal"] ?? identityUrl;
        ...
        .AddOpenIdConnect(options =>
        {
            ...
            // Allow correlation cookies to work over plain HTTP (K8s without TLS)
            options.CorrelationCookie.SameSite = SameSiteMode.Lax;
            options.CorrelationCookie.SecurePolicy = CookieSecurePolicy.None;
            options.NonceCookie.SameSite = SameSiteMode.Lax;
            options.NonceCookie.SecurePolicy = CookieSecurePolicy.None;

            // When IdentityUrlExternal differs from IdentityUrl, rewrite browser redirects
            // so the browser hits the external URL while backchannel uses internal K8s DNS.
            if (!string.Equals(identityUrl, identityUrlExternal, StringComparison.Ordinal))
            {
                options.Events = new Microsoft.AspNetCore.Authentication.OpenIdConnect.OpenIdConnectEvents
                {
                    OnRedirectToIdentityProvider = context =>
                    {
                        context.ProtocolMessage.IssuerAddress = context.ProtocolMessage.IssuerAddress
                            .Replace(identityUrl, identityUrlExternal);
    ...
    private static void AddAIServices(this IHostApplicationBuilder builder)
    {
        ...
        {
            // Use our ONNX Inference service as an OpenAI-compatible endpoint
            var openAiClient = new OpenAI.OpenAIClient(
                new System.ClientModel.ApiKeyCredential("unused"),
                new OpenAI.OpenAIClientOptions { Endpoint = new Uri(inferenceUrl) });
            var innerClient = openAiClient.GetChatClient("phi-4-mini").AsIChatClient();
            chatClientBuilder = builder.Services.AddChatClient(innerClient);
        }
        ...
        // Tool invocation is handled manually in ChatState.AddUserMessageAsync
        // to allow skipping the second LLM call when product cards are rendered directly.

        // NOTE: Prefix KV-cache warmup is performed inside the inference service
        // itself (see Inference/CanonicalWarmup.cs + Inference/Program.cs startup
        // hook), not here. The previous WebApp-side InferenceWarmupService was
        // removed because it raced with Kubernetes RollingUpdate of the inference
        // Deployment — WebApp would prime the OUTGOING inference pod, then that
        // pod terminated taking the warm cache to the grave. Keeping warmup
        // co-located with the model means every new inference pod self-primes
        // regardless of webapp lifecycle, scale events, or pod restarts.
    }
